...
namespace Toreo {
    ...
    buttons_(5)
  {
    ...
    button.u = 8.0f;
    button.v = 1954.0f;
    button.id = static_cast<float>(Visualizer::Menu::ZOOM_IN);
    buttons_[0] = button;

    button.left = 94.0f;
    button.v = 1859.0f;
    button.id = static_cast<float>(Visualizer::Menu::ISOMETRIC_VIEW);
    buttons_[1] = button;

    button.left = 188.0f;
    button.v = 1764.0f;
    button.id = static_cast<float>(Visualizer::Menu::TOP_VIEW);
    buttons_[2] = button;

    button.left = 282.0f;
    button.v = 1668.0f;
    button.id = static_cast<float>(Visualizer::Menu::ZOOM_OUT);
    buttons_[3] = button;

    button.left = 0.0f;
    button.v = 1553.0f;
    button.id = static_cast<float>(Visualizer::Menu::PEM_LOGO);
    buttons_[4] = button;

    set_buffer();
  }

  void GUIMenu::change_button_state(const Visualizer::Menu::ButtonType button_id,
                                    const Visualizer::Button::State state){
    const std::size_t id{static_cast<std::size_t>(button_id)};
    const float width{MENU_BUTTON_WIDTH}, height{MENU_BUTTON_HEIGHT};
    ...
    if(buttons_[id].state != state){
      ...
      const float i{buttons_[id].id}, p{static_cast<float>(Visualizer::GUIid::MENU)};
  ...
  void GUIMenu::restart_buttons(){
    for(Visualizer::Button::Button &button : buttons_)
      if(button.state != Visualizer::Button::NORMAL)
        change_button_state(static_cast<Visualizer::Menu::ButtonType>(button.id),
                            Visualizer::Button::NORMAL);
  }

  void GUIMenu::click_event(const Visualizer::Menu::ButtonType button_id){
    switch(button_id){
    case Visualizer::Menu::ZOOM_IN:
      core_->camera_zoom_in();
    break;
    case Visualizer::Menu::ZOOM_OUT:
      core_->camera_zoom_out();
    break;
    case Visualizer::Menu::ISOMETRIC_VIEW:
      core_->camera_view_isometric();
    break;
    case Visualizer::Menu::TOP_VIEW:
  ...
  void GUIMenu::set_buffer(){
    const float width{MENU_BUTTON_WIDTH}, height{MENU_BUTTON_HEIGHT};
    const float p{static_cast<float>(Visualizer::GUIid::MENU)};
