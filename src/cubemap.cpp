...
namespace Toreo {
    ...
    i_uv_(2)
  {
    ...
    runner_ = boost::thread(boost::bind(&Cubemap::load_images, this));
    runner_.detach();
  }
    ...
    if(protector_.try_lock()){
      ...
      if(is_ready_){
        ...
        */
        ...
        // Deleting unnecessary data buffers
        delete buffer_cube_;
        ...
        core_->message_handler("Some/all files for the cubemap were not found", Visualizer::ERROR);
      }
    }
  }

  void Cubemap::write_data_opengl(const Visualizer::ImageFile &image, const int level){
  ...
  void Cubemap::prepare_cube(){
    ...
    GLsizei stride_size_long{sizeof(GLfloat) * 8};
    GLsizei stride_size_short{sizeof(GLfloat) * 3};

    GLint offset{0};
    buffer_cube_->enable(i_position_);
    buffer_cube_->attributte_buffer(i_position_, _3D, offset, stride_size_long);

    offset += stride_size_short;
    buffer_cube_->enable(i_normal_);
    buffer_cube_->attributte_buffer(i_normal_, _3D, offset, stride_size_long);

    offset += stride_size_short;
    buffer_cube_->enable(i_uv_);
    buffer_cube_->attributte_buffer(i_uv_, _2D, offset, stride_size_long);

    buffer_cube_->vertex_release();
  }

  void Cubemap::prepare_quad(){
    float vertices[] = {
    ...
    GLsizei stride_size_medium{sizeof(GLfloat) * 5};
    ...
    buffer_squad_->attributte_buffer(i_position_, _3D, 0, stride_size_medium);

    buffer_squad_->enable(1);
    buffer_squad_->attributte_buffer(1, _2D, sizeof(GLfloat) * 3, stride_size_medium);
