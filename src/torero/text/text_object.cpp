...
namespace torero {
  namespace text {
    TextObject::TextObject(torero::gl::Shader *shader,
                           const torero::camera::Dimensionality dimensions) :
      shader_(shader),
      buffer_(true),
      font_(nullptr),
      type_(torero::text::Complexity::Simple),
      weight_(torero::text::Weight::Normal),
      is_3D_(dimensions == torero::camera::Dimensionality::ThreeDimensional),
      ...
      has_border_int_(0),
      has_color_int_(0),
      vertical_alignment_(torero::text::Alignment::Top),
      horizontal_alignment_(torero::text::Alignment::Left),
      type_size_(sizeof(torero::text::SimpleShader)),
      data_size_(0),
    ...
    void TextObject::set_input(const std::vector<torero::text::Simple> *text){
      if(type_ == torero::text::Complexity::Simple)
        text_simple_ = text;
      else{
        text_simple_ = text;
        text_colored_ = nullptr;
    ...
    void TextObject::set_input(const std::vector<torero::text::Colored> *text){
      if(type_ == torero::text::Complexity::Medium)
        text_colored_ = text;
      else{
        text_simple_ = nullptr;
        text_colored_ = text;
        type_ = torero::text::Complexity::Medium;
        type_size_ = sizeof(torero::text::ColoredShader);
        data_size_ = 0;
        has_orientation_ = 1.0f;
      }
    }

    void TextObject::set_font(torero::text::FontLoader *font){
      font_ = font;
    }
    ...
    void TextObject::set_size(const unsigned int size){
      size_softness_weight_[0] = to_float(size);
      set_weight(weight_);
    }
    ...
    void TextObject::set_vertical_alignment(const torero::text::Alignment alignment){
      vertical_alignment_ = alignment;
    }

    void TextObject::set_horizontal_alignment(const torero::text::Alignment alignment){
      horizontal_alignment_ = alignment;
    }

    void TextObject::set_weight(const torero::text::Weight weight){
      weight_ = weight;
      float multiplier;
      switch(static_cast<unsigned int>(weight)){
        case static_cast<unsigned int>(torero::text::Weight::Lighter):
          multiplier = 0.03f;
        break;
        case static_cast<unsigned int>(torero::text::Weight::Light):
          multiplier = 0.06f;
        break;
        case static_cast<unsigned int>(torero::text::Weight::Bold):
          multiplier = 0.15f;
        break;
        case static_cast<unsigned int>(torero::text::Weight::Bolder):
          multiplier = 0.2f;
        break;
        default:
          multiplier = 0.1f;
    ...
    bool TextObject::set_border_offset(const unsigned int x, const unsigned int y){
      border_width_softness_offset_[2] = to_float(x);
      border_width_softness_offset_[3] = to_float(y);
    }
    ...
    bool TextObject::update(){
      ...
      if(no_error){
        if(type_ == torero::text::Complexity::Medium){
          ...
          std::vector<torero::text::ColoredShader> buffer_data(1);
    ...
    void TextObject::text_metrics(const std::string &text, algebraica::vec2f *offset,
                                  ...
                                  std::vector<torero::text::SimpleShader> *simple,
                                  std::vector<torero::text::ColoredShader> *colored){
