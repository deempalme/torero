...
namespace torero {
  namespace gui {
    Options::Options(torero::Core *core, torero::gl::Shader *color_shader,
    ...
    void Options::change_button_state(const torero::gui::menu::ButtonType button_id,
                                      const torero::gui::button::State state){
      const std::size_t id{static_cast<std::size_t>(button_id)};
      const float width{torero::kMenuButtonWidth}, height{torero::kMenuButtonHeight};
      ...
      if(buttons_[id].state != state){
        ...
        *has_changed_ = true;
        ...
        if(state == torero::gui::button::Hover || state == torero::gui::button::Click)
          core_->set_cursor(torero::gui::Cursor::Hand);
        else
          core_->set_cursor(torero::gui::Cursor::Normal);
      }
    }

    void Options::restart_buttons(){
    ...
    void Options::click_event(const torero::gui::menu::ButtonType button_id){
      switch(button_id){
        case torero::gui::menu::ZoomIn:
          core_->camera()->zoom_in();
        break;
    ...
    void Options::set_offset(const int top, const int left){
      menu_offset_(static_cast<float>(top), static_cast<float>(left));
    }

    void Options::set_alignment(const float top, const float left){
