...
namespace torero {
  namespace gui {
    TitleBar::TitleBar(torero::Core *core, torero::gl::Shader *color_shader,
      ...
      buttons_(5)
    {
      ...
      button.v = 1515.0f;
      button.width = -120.0f;
      button.height = torero::k_title_height;
      button.id = static_cast<float>(torero::gui::title::Title);
      buttons_[0] = button;
      ...
      button.u = 8.0f;
      button.v = 1485.0f;
      button.width = torero::k_title_button_width;
      button.id = static_cast<float>(torero::gui::title::Close);
      buttons_[1] = button;

      button.left = -30.0f;
      button.v = 1455.0f;
      button.id = static_cast<float>(torero::gui::title::Maximize);
      buttons_[2] = button;

      button.left = -60.0f;
      button.v = 1425.0f;
      button.id = static_cast<float>(torero::gui::title::Minimize);
      buttons_[3] = button;

      button.left = 0.0f;
      button.v = 1395.0f;
      button.id = static_cast<float>(torero::gui::title::Options);
      buttons_[4] = button;

      set_buffer();
    }

    void TitleBar::change_button_state(const torero::gui::title::ButtonType button_id,
      ...
      if(buttons_[id].state != state){
        ...
        const float width2 = (width <= 0.0f)? core_->size().point.x + width : width;
        const float plus{static_cast<float>(state) * torero::k_title_button_width};
        ...
        const float u{buttons_[id].u + plus}, u_plus{u + torero::k_title_button_width};
        const float v{buttons_[id].v}, v_plus{height + v};
        const float i{buttons_[id].id}, p{static_cast<float>(torero::gui::object::TitleBar)};
        ...
        if(state == torero::gui::button::Hover || state == torero::gui::button::Click)
          if(button_id == torero::gui::title::Title)
    ...
    void TitleBar::restart_buttons(){
      for(torero::gui::button::Button &button : buttons_)
        if(button.state != torero::gui::button::Normal)
          change_button_state(static_cast<torero::gui::title::ButtonType>(button.id),
                              torero::gui::button::Normal);
    }

    void TitleBar::click_event(const torero::gui::title::ButtonType button_id,
                                  const torero::gui::mouse::Event event){
      if(event == torero::gui::mouse::Click){
        if(button_id == torero::gui::title::Title)
          core_->camera().blocked(true);
      }else
        switch(static_cast<unsigned int>(button_id)){
          case static_cast<unsigned int>(torero::gui::title::Close):
            core_->close();
          break;
          case static_cast<unsigned int>(torero::gui::title::Maximize):
            core_->maximize();
          break;
          case static_cast<unsigned int>(torero::gui::title::Minimize):
            core_->minimize();
          break;
          case static_cast<unsigned int>(torero::gui::title::Options):
    ...
    void TitleBar::set_buffer(){
      const float width{torero::k_title_button_width}, height{torero::k_title_height};
      const float p{static_cast<float>(torero::gui::object::TitleBar)};
      ...
      const float left_plus[5] = {core_->size().point.x + buttons_[0].width + buttons_[0].left,
