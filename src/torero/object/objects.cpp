...
namespace torero {
  namespace object {
    Objects::Objects(torero::gl::Shader *shader_program,
      ...
      solid_data_size_(0),
      arrow_data_size_(0),
      hollow_type_size_(sizeof(torero::object::ShaderHollow)),
      solid_type_size_(sizeof(torero::object::ShaderSolid)),
      buffer_size_(sizeof(torero::object::Buffer))
    {
      initialize();
    }

    void Objects::change_input(const std::vector<torero::object::Object> *objects){
      object_ = objects;
      hollow_data_size_ = 0;
      solid_data_size_ = 0;
      arrow_data_size_ = 0;
    ...
    bool Objects::update(){
      ...
      if(no_error){
        std::vector<torero::object::ShaderSolid> solid_data(0);
        std::vector<torero::object::ShaderHollow> hollow_data(0);
        std::vector<torero::object::ShaderSolid> arrow_data(0);

        for(const torero::object::Object &object : *object_){
          if(object.solid){
            torero::object::ShaderSolid datum;
            ...
            if(type_ == torero::object::Square || type_ == torero::object::Circle)
              datum.scale[1] = 1.0f;
            solid_data.push_back(datum);
          }else{
            torero::object::ShaderHollow datum;
            datum.position(-object.position.point.y, object.position.point.z,
                           -object.position.point.x);
            datum.rotation( object.orientation.axes.y,-object.orientation.axes.z,
                            -object.orientation.axes.x, object.orientation.axes.w);
    ...
    void Objects::initialize(){
      ...
      switch(type_){
        case torero::object::Cylinder:
          type_size_hollow_ = 672;
          type_size_solid_ = 144;
        break;
        case torero::object::Box:
          type_size_hollow_ = 288;
          type_size_solid_ = 36;
        break;
        case torero::object::Square:
          type_size_hollow_ = 96;
          type_size_solid_ = 6;
        break;
        case torero::object::Circle:
