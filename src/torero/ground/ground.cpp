...
namespace torero {
  namespace ground {
    Ground::Ground(torero::gl::Shader *ground_shader) :
    ...
    void Ground::change_input(const std::vector<torero::ground::Ground2D> *ground){
      restart();
      ground_2D_ = ground;
      type_size_ = sizeof(torero::ground::Shader2D);
      is_2D_ = 1;
    }

    void Ground::change_input(const std::vector<torero::ground::Ground3D> *ground){
      restart();
      ground_3D_ = ground;
      type_size_ = sizeof(torero::ground::Shader3D);
    }

    void Ground::change_input(const std::vector<torero::ground::Grid> *ground){
      restart();
      ground_grid_ = ground;
      type_size_ = sizeof(torero::ground::GridShader);
      is_grid_ = 1;
    }

    void Ground::change_input(const std::vector<torero::ground::Free2D> *ground){
      restart();
      free_ground_2D_ = ground;
      type_size_ = sizeof(torero::ground::Free2D);
      is_free_ = 1;
      is_2D_ = 1;
    }

    void Ground::change_input(const std::vector<torero::ground::Free3D> *ground){
      restart();
      free_ground_3D_ = ground;
      type_size_ = sizeof(torero::ground::Free3D);
      is_free_ = 1;
    }

    void Ground::change_input(const std::vector<torero::ground::FreePolar2D> *ground){
      restart();
      polar_ground_2D_ = ground;
      type_size_ = sizeof(torero::ground::FreePolar2D);
      is_free_ = 1;
      is_polar_ = 1;
      is_2D_ = 1;
    }

    void Ground::change_input(const std::vector<torero::ground::FreePolar3D> *ground){
      restart();
      polar_ground_3D_ = ground;
      type_size_ = sizeof(torero::ground::FreePolar3D);
      is_free_ = 1;
      is_polar_ = 1;
    }
      ...
      if(no_error){
        ...
        if(ground_2D_){
          ...
          if(ground_2D_->size() >= static_cast<std::size_t>(data_size_)){
            std::vector<torero::ground::Shader2D> ground(data_size_);
          ...
          if(ground_3D_->size() >= static_cast<std::size_t>(data_size_)){
            std::vector<torero::ground::Shader3D> ground(data_size_);
          ...
          if(ground_grid_->size() >= static_cast<std::size_t>(data_size_)){
            std::vector<torero::ground::GridShader> ground(data_size_);
