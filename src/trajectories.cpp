...
namespace torero {
  Trajectories::Trajectories(Shader *shader_program,
                             const std::vector<torero::Trajectory> *trajectories) :
    ...
    secondary_model_(),
    identity_matrix_(),
    type_size_(sizeof(torero::TrajectoryShader)),
    data_size_(0)
  {
    initialize();
  }

  void Trajectories::change_input(const std::vector<torero::Trajectory> *trajectories){
    trajectories_ = trajectories;
  }
  ...
  bool Trajectories::update(){
    ...
    if(no_error){
      size_t size{0};
      for(const torero::Trajectory &line : *trajectories_)
        if(line.size() > 1)
          size += line.size();

      std::vector<torero::TrajectoryShader> trajectories(0);
      trajectories.reserve(size + trajectories_->size() * 2);
      torero::TrajectoryShader first, second;
      ...
      for(const torero::Trajectory &line : *trajectories_){
        if((total = line.size()) > 1){
          ...
          std::vector<torero::TrajectoryShader> sub_trajectory(total + 3);
