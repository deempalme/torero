...
namespace Toreo {
  TextObject::TextObject(Shader *shader) :
    shader_(shader),
    buffer_(true),
    font_(nullptr),
    type_(Visualizer::SIMPLE),
    text_simple_(nullptr),
    text_colored_(nullptr),
    text_extended_(nullptr),
    primary_model_(nullptr),
    secondary_model_(),
    identity_matrix_(),
    color_(),
    type_size_(sizeof(Visualizer::TextSimple)),
    data_size_(0)
  {
    initialize();
  }
  ...
  void TextObject::set_input(const std::vector<Visualizer::TextColored> *text){
    if(type_ == Visualizer::MEDIUM)
      text_colored_ = text;
    else{
      text_simple_ = nullptr;
      text_extended_ = nullptr;
      type_ = Visualizer::MEDIUM;
      type_size_ = sizeof(Visualizer::TextColored);
      data_size_ = 0;
    }
  }

  void TextObject::set_input(const std::vector<Visualizer::TextExtended> *text){
    if(type_ == Visualizer::SIMPLE)
      text_extended_ = text;
    else{
      text_simple_ = nullptr;
      text_colored_ = nullptr;
      type_ = Visualizer::COMPLEX;
      type_size_ = sizeof(Visualizer::TextExtended);
      data_size_ = 0;
    }
  }
