...
layout(location = 5) in float i_angle;     // vertex rotation angle in Y axis
layout(location = 6) in vec3 i_color;      // vertex emission color
...
void main()
{
  ...
  o_TBN = mat3(normalize(quaternion_rotation(u_rotation, i_tangent)),
               normalize(quaternion_rotation(u_rotation, i_bitangent)),
