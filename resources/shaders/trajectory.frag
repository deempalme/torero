...
vec3 calculate_point_light(const vec3 light, const vec3 color,
  ...
  vec3 albedo = f_color.rgb * texture(u_diffuse, f_uv).rgb;
  // ambient
...
void main()
{
  ...
  frag_color = vec4(color, f_color.a * texture(u_diffuse, f_uv).a);
}
