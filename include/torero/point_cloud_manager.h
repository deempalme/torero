...
namespace torero {
  ...
  public:
     ...
     * {const algebraica::mat4f*} transformation_matrix = Address to the transformation matrix
     * that defines the coordinate system's origin and orientation.
     * {const torero::ColorMode} color_mode = see **color mode** *section*.
     * {const bool} visible = visibility of this point cloud.
     * {const float} point_size = point size for each point in the cloud.
     * {const float} maximum_intensity_value = maximum value that the intensity could have.
     *
    ...
    PCMid add(const std::vector<torero::PointXYZI> *point_cloud,
              const std::string name,
              const algebraica::mat4f *transformation_matrix = nullptr,
              const torero::ColorMode color_mode = torero::ColorMode::ColorMap,
              const bool visible = true,
              const float point_size = 1.0f,
              const float maximum_intensity_value = 1.0f);
    /*
     ...
     *
     * torero::ColorMode::INTENSITY -> Colorize points multiplying *intensity* and
     *                                     first color in *color_palete*
     * torero::ColorMode::SOLID     -> Colorize all points using only the first color in
     *                                     *color_palete*
     * torero::ColorMode::COLORMAP  -> Colorize points interpolating the *color_palete*
     *                                     using the point intensity
     * torero::ColorMode::DATA      -> Use this when you specify colors for each point
     *
     * **Arguments**
     * {PCMid} id = **id** of the point cloud you want to modify.
     * {const torero::ColorMode} color_mode = new color mode.
     *
     * **Returns**
     * {bool} Returns `false` if the point cloud with **id** was **not** found.
     *
  ...
  private:
    ...
    Shader *shader_;
    GLint u_pv_;
    std::vector<torero::PointCloudElement> point_clouds_;
