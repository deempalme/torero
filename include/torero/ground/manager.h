...
namespace torero {
  ...
  namespace ground {
     ...
     *
     */
      GMid add(const std::vector<torero::ground::Grid> *ground,
     ...
     *
     */
      GMid add(const std::vector<torero::ground::Ground2D> *ground,
               const std::string name,
               const float width = 100.0f,
               const float length = 100.0f,
               const unsigned int number_of_elements_through_width = 100u,
     ...
     *
     */
      GMid add(const std::vector<torero::ground::Ground3D> *ground,
               const std::string name,
               const float width = 100.0f,
               const float length = 100.0f,
               const unsigned int number_of_elements_through_width = 100u,
     ...
     *
     * This will add a new ground with values type `torero::Free2D`, with this **data
     * type** you can define a **position** and **size** for each segment. You must also
     * define a **RGBA color** to color each segment. It will return the ground's **ID**,
     * this will be useful if you want to modify properties or values of the created ground.
     *
     * **Arguments**
     * {const std::vector<torero::Free2D>*} ground = Address to the ground data
     * (see data types for more information).
     * {const std::string} name = Title to display for this ground.
     * {const algebraica::mat4f*} transformation_matrix = Address to the transformation matrix
     * that defines the coordinate system's origin and orientation.
     ...
     *
     */
      GMid add(const std::vector<torero::ground::Free2D> *ground,
               const std::string name,
               const algebraica::mat4f *transformation_matrix = nullptr,
               const bool ground_visible = true);
      /*
     ...
     *
     * This will add a new ground with values type `torero::Free3D`, with this **data
     ...
     *
     * **Arguments**
     * {const std::vector<torero::Free3D>*} ground = Address to the ground data
     * (see data types for more information).
     * {const std::string} name = Title to display for this ground.
     * {const algebraica::mat4f*} transformation_matrix = Address to the transformation matrix
     * that defines the coordinate system's origin and orientation.
     ...
     *
     */
      GMid add(const std::vector<torero::ground::Free3D> *ground,
               const std::string name,
               const algebraica::mat4f *transformation_matrix = nullptr,
               const bool ground_visible = true);
      /*
     ...
     *
     * This will add a new ground with values type `torero::FreePolar2D`, with this **data
     * type** you can define a **distance**, **angle** and **size** for each segment. You must also
     * define a **RGBA color** to color each segment. It will return the ground's **ID**,
     * this will be useful if you want to modify properties or values of the created ground.
     *
     * **Arguments**
     * {const std::vector<torero::FreePolar2D>*} ground = Address to the ground data
     * (see data types for more information).
     * {const std::string} name = Title to display for this ground.
     * {const algebraica::mat4f*} transformation_matrix = Address to the transformation matrix
     * that defines the coordinate system's origin and orientation.
     ...
     *
     */
      GMid add(const std::vector<torero::ground::FreePolar2D> *ground,
               const std::string name,
               const algebraica::mat4f *transformation_matrix = nullptr,
               const bool ground_visible = true);
      /*
     ...
     *
     * This will add a new ground with values type `torero::FreePolar3D`, with this **data
     * type** you can define a **position**, **size** and **height** for each segment. You must
     * also define a **RGBA color** to color each segment. It will return the ground's **ID**,
     * this will be useful if you want to modify properties or values of the created ground.
     *
     * **Arguments**
     * {const std::vector<torero::FreePolar3D>*} ground = Address to the ground data
     * (see data types for more information).
     * {const std::string} name = Title to display for this ground.
     * {const algebraica::mat4f*} transformation_matrix = Address to the transformation matrix
     * that defines the coordinate system's origin and orientation.
     ...
     *
     */
      GMid add(const std::vector<torero::ground::FreePolar3D> *ground,
               const std::string name,
               const algebraica::mat4f *transformation_matrix = nullptr,
               const bool ground_visible = true);
      /*
     ...
     *
     */
      bool change_input(GMid id, const std::vector<torero::ground::Ground2D> *ground);
      /*
     ...
     *
     */
      bool change_input(GMid id, const std::vector<torero::ground::Ground3D> *ground);
      /*
     ...
     * **Arguments**
     * {GMid} id = **id** of the ground you want to modify.
     * {const std::vector<torero::Free2D>*} trajectories = new address to a 3D ground's
     * data.
     *
     * **Returns**
     * {bool} Returns `false` if the ground with **id** was **not** found.
     *
     */
      bool change_input(GMid id, const std::vector<torero::ground::Free2D> *ground);
      /*
     ...
     * **Arguments**
     * {GMid} id = **id** of the ground you want to modify.
     * {const std::vector<torero::Free3D>*} trajectories = new address to a 3D ground's
     * data.
     *
     * **Returns**
     * {bool} Returns `false` if the ground with **id** was **not** found.
     *
     */
      bool change_input(GMid id, const std::vector<torero::ground::Free3D> *ground);
      /*
     ...
     * **Arguments**
     * {GMid} id = **id** of the ground you want to modify.
     * {const std::vector<torero::FreePolar2D>*} trajectories = new address to a 3D ground's
     * data.
     *
     * **Returns**
     * {bool} Returns `false` if the ground with **id** was **not** found.
     *
     */
      bool change_input(GMid id, const std::vector<torero::ground::FreePolar2D> *ground);
      /*
     ...
     * **Arguments**
     * {GMid} id = **id** of the ground you want to modify.
     * {const std::vector<torero::FreePolar3D>*} trajectories = new address to a 3D ground's
     * data.
     *
     * **Returns**
     * {bool} Returns `false` if the ground with **id** was **not** found.
     *
     */
      bool change_input(GMid id, const std::vector<torero::ground::FreePolar3D> *ground);
      /*
    ...
    private:
      ...
      torero::gl::Shader *ground_shader_;
      GLint u_point_light_ground_, u_point_light_color_ground_, u_directional_light_ground_;
      GLint u_directional_light_color_ground_, u_camera_position_ground_;

      std::vector<torero::ground::Element> grounds_;

      torero::ground::LineGrid *grid_;
      torero::gl::Shader *line_shader_;
      bool grid_visibility_;

      boost::signals2::connection connection_camera_;
      boost::signals2::connection connection_update_, connection_draw_;
    };
  }
}
