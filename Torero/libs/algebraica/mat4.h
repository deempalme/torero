...
public:
  ...
  mat4<T>& rotate(const T angle, const vec3<T> &v){
    T const c{cos(angle)}, t{T(1) - c};
    T const s{sin(angle)};

    T x{v[0]}, y{v[1]}, z{v[2]}, lenght{sqrt(x * x + y * y + z * z)};
  ...
  mat4<T>& rotate(const T pitch, const T yaw, const T roll){
    const T cosA{cos(pitch)}, sinA{sin(pitch)};
    const T cosB{cos(yaw)},   sinB{sin(yaw)};
    const T cosC{cos(roll)},  sinC{sin(roll)};
  ...
  mat4<T>& look_at(const vec3<T> eye, const vec3<T> center, const vec3<T> up){
    ...
    const vec3<T> s(vec3<T>::normalize(vec3<T>::cross(f, up)));
   ...
   */
  vec4<T>& operator[](const unsigned int index){
    assert(index >= 0 && index < 4);
    return m_[index];
  }
  const vec4<T>& operator[](const unsigned int index) const{
    assert(index >= 0 && index < 4);
    return m_[index];
  }
  /*
  ...
  friend ostream& operator<<(ostream &os, const mat4<T> &m){
    return os << "mat4(" << setprecision(4) << fixed <<
